...
function setup() {
  createCanvas(1366, 200);

 
  
  trex = createSprite(displayWidth-700,160,20,50);
  trex.addAnimation("running", trex_running);
  trex.addAnimation("collided", trex_collided);
  trex.scale = 0.5;
  trex.velocityX = 1;
  
  ground = createSprite(-500,180,1366/2,20);
  ground.addImage("ground",groundImage);
  ground.x = ground.width /2;
  
  gameOver = createSprite(camera.position.x + 500,50);
  gameOver.addImage(gameOverImg);
  
  restart = createSprite(camera.position.x + 500,100);
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(trex + 600,120,40,10);
